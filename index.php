<?php
	...
	include('config.php');
	//Side bar info
	$total_loans      = $db->selectSum("loans", "capital", "active = 1");
	$total_payments   = $db->selectSum("payments", "capital + interest", "active = 1");
	$total_clients    = $db->countObjects("clients", null);

	//Main content
	$payments = $db->selectObjectsBySql("SELECT DATE_FORMAT(paymentDate, '%M_%d_%Y') dateCollection, sum(capital + interest) totalCollection FROM payments WHERE active = 1 GROUP BY DATE_FORMAT(paymentDate, '%M-%D-%Y') ORDER BY paymentDate DESC");
	...
	<div class="row-fluid">
		<div class="span3">
			 ...
			 <ul class="unstyled sidebar"> 
				...
				<li>Loans: <?php echo formatToAmount($total_loans); ?></li>
			...
			<table class="table table-hover table-striped ">
				...
				<?php foreach($data as $item) : ?>
					<tr>
						...
						<td><a data-toggle="modal" href="forms/loanSummary.php?date=<?php echo date("F-d-Y", strtotime($item->date)); ?>" data-target="#loanSummary"><?php echo $item->loan; ?></a></td>
					</tr>
				<?php endforeach ?>
				</tbody>	
			</table>     
