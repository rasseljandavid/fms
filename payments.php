<?php
	...
	for($i = 0; $i < count($payments); $i++) {
		...
		$payments[$i]->name        = "<a href='client.php?id={$payments[$i]->client_id}#payments'>{$payments[$i]->firstname} {$payments[$i]->lastname}</a>";
		...
		$payments[$i]->payment     = formatToAmount($loan->capital) . " Loan (" . date("F d, Y", strtotime($loan->loanDate)) . ")";
		$payments[$i]->category    = $db->selectValue("categories","category","id = {$loan->category_id}");
	...
	<div class="row-fluid">
		<div class="span3">
			...
			<ul class="unstyled sidebar"> 
				<li class="nav-header">Total Payments</li> 
