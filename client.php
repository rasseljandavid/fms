	...
	<div class="row-fluid">
		<div class="span3">
			...
			<ul class="nav nav-tabs nav-stacked">
				...
				<li><a href="forms/loan.php?client_id=<?php echo $client->id; ?>" data-toggle="modal" data-target="#newLoan">Add Loan</a></li>
				<li><a data-toggle="modal" href="forms/payment.php?client_id=<?php echo $client->id; ?>" data-target="#newPayment">Add Payment</a></li>
		...
		<div class="span9">
			...
			<div class="tab-content">
							...
							<tbody>
								<tr>
									...
									<td>
										...
										<p style="margin-left: 15px;">Remaining Balance: <strong> <?php echo formatToAmount($remaining_balance); ?> </strong></p>
				...
				<div class="content">
					<h4 class="tab-header">Loans of <?php echo $client->firstname; ?></h4>
					<table class="table table-bordered table-hover table-striped loans">
						<thead>
						<tr>
							<th>Date</th>
							<th>Capital</th>
						...
						<?php foreach($loans as $item) : ?>
							<?php $total_daily_payment = 0; ?>
							<tr class="<?php if($item->paid) echo "muted"; ?>">
								<td><?php echo date('M d, Y', strtotime($item->loanDate)); ?></td>

								<td><?php echo formatToAmount($item->capital); ?></td>
								<td><?php echo formatToAmount($item->capital * $item->interest); ?></td>
								<td><?php echo $item->monthsToPay; ?></td>
								<td><?php echo formatToAmount(($item->capital * $item->interest) * ($item->monthsToPay) + $item->capital); ?></td>
								<td><?php echo formatToAmount(getLoanBalance($item->id)); ?></td>
								<td class="actions">
									<a data-toggle="modal" href="forms/loan.php?loan_id=<?php echo $item->id; ?>" data-target="#newLoan">
										Edit
									</a> <a href="delete.php?id=<?php echo $item->id; ?>&type=loans" class="icon-trash " onclick="return confirm('Are you sure you want delete this loan?');"></a>
								</td>
							</tr>
							<?php endforeach; ?>
						</tbody>
				...
				<div class="content">
				<h4 class="tab-header">Payments of <?php echo $client->firstname; ?></h4>
